...
const courseLessonKindInput = z.enum(["text", "video", "audio", "image", "download", "quiz"]);
const quizInput = z.object({ prompt: z.string().min(1).max(4000), choices: z.array(z.string().min(1).max(500)).min(2).max(8), correctAnswerIndex: z.number().int().min(0).max(7), explanation: z.string().max(4000).optional(), passingScore: z.number().int().min(0).max(100).optional() });
const courseLessonInput = z.object({ id: z.number().int().optional(), courseId: z.number().int(), moduleId: z.number().int().nullable().optional(), title: z.string().min(2).max(255), kind: courseLessonKindInput, body: z.string().max(20000).optional(), videoUrl: z.string().url().max(1024).optional().or(z.literal("")), fileUrl: z.string().url().max(1024).optional().or(z.literal("")), mediaAssetId: z.number().int().nullable().optional(), thumbnailAssetId: z.number().int().nullable().optional(), galleryAssetIds: z.array(z.number().int()).max(24).optional(), resourceAssetIds: z.array(z.number().int()).max(40).optional(), durationSeconds: z.number().int().min(0).max(86_400).nullable().optional(), quiz: quizInput.nullable().optional(), isPublished: z.boolean().default(true), isLocked: z.boolean().default(false), dripDays: z.number().int().min(0).max(3650).default(0), prerequisiteLessonId: z.number().int().nullable().optional(), isPreview: z.boolean().default(false) });
...
async function validateCourseLessonOwnership(db: Awaited<ReturnType<typeof requireDb>>, creatorId: number, courseId: number, input: z.infer<typeof courseLessonInput>) {
  if (input.moduleId) {
    const module = (await db.select().from(courseModules).where(and(eq(courseModules.id, input.moduleId), eq(courseModules.courseId, courseId))).limit(1))[0];
    if (!module) throw new TRPCError({ code: "NOT_FOUND", message: "Course module not found." });
  }
  if (input.prerequisiteLessonId) {
    const prerequisite = (await db.select().from(lessons).where(and(eq(lessons.id, input.prerequisiteLessonId), eq(lessons.courseId, courseId))).limit(1))[0];
    if (!prerequisite || prerequisite.id === input.id) throw new TRPCError({ code: "BAD_REQUEST", message: "Choose another lesson from this course as the prerequisite." });
  }
  const assetIds = [input.mediaAssetId, input.thumbnailAssetId, ...(input.galleryAssetIds ?? []), ...(input.resourceAssetIds ?? [])].filter((id): id is number => id !== null && id !== undefined);
  await Promise.all(assetIds.map((id) => requireOwnedAsset(db, creatorId, id)));
  if (input.quiz && input.quiz.correctAnswerIndex >= input.quiz.choices.length) throw new TRPCError({ code: "BAD_REQUEST", message: "Choose a valid correct quiz answer." });
}

function studentLessonAvailability(lesson: { isPublished: boolean; isLocked: boolean; dripDays: number; prerequisiteLessonId: number | null }, module: { status: "draft" | "published"; isVisible: boolean; dripDays: number } | undefined, enrollment: { enrolledAt: Date }, completedLessonIds: number[]) {
  if (!lesson.isPublished || (module && (module.status !== "published" || !module.isVisible))) return { isAvailable: false, availableAt: null, reason: "This lesson is not available yet." };
  const delayDays = Math.max(lesson.dripDays ?? 0, module?.dripDays ?? 0);
  const availableAt = delayDays ? new Date(new Date(enrollment.enrolledAt).getTime() + delayDays * 86_400_000) : null;
  if (availableAt && availableAt > new Date()) return { isAvailable: false, availableAt, reason: "This lesson is scheduled for later." };
  if (lesson.isLocked && lesson.prerequisiteLessonId && !completedLessonIds.includes(lesson.prerequisiteLessonId)) return { isAvailable: false, availableAt, reason: "Complete the required lesson first." };
  return { isAvailable: true, availableAt, reason: null };
}
...
export const appRouter = router({
  ...
  courses: router({
    ...
    create: protectedProcedure.input(z.object({ title: z.string().min(2).max(255), description: z.string().max(8000).optional(), learningOutcomes: z.array(z.string().min(1).max(240)).max(20).optional(), thumbnailAssetId: z.number().int().nullable().optional(), coverAssetId: z.number().int().nullable().optional(), settings: z.object({ instructorName: z.string().max(160).optional(), welcomeMessage: z.string().max(1200).optional(), certificateEnabled: z.boolean().optional() }).optional() })).mutation(async ({ ctx, input }) => {
      const db = await requireDb(); const creator = await ownedCreator(ctx);
      if (input.thumbnailAssetId) await requireOwnedAsset(db, creator.id, input.thumbnailAssetId); if (input.coverAssetId) await requireOwnedAsset(db, creator.id, input.coverAssetId);
      const result = await db.insert(courses).values({ ...input, creatorId: creator.id }); return Number(result[0].insertId);
    }),
    update: protectedProcedure.input(z.object({ id: z.number().int(), title: z.string().min(2).max(255), description: z.string().max(8000).optional(), status: z.enum(["draft", "published"]), learningOutcomes: z.array(z.string().min(1).max(240)).max(20).optional(), thumbnailAssetId: z.number().int().nullable().optional(), coverAssetId: z.number().int().nullable().optional(), settings: z.object({ instructorName: z.string().max(160).optional(), welcomeMessage: z.string().max(1200).optional(), certificateEnabled: z.boolean().optional() }).optional() })).mutation(async ({ ctx, input }) => {
      const db = await requireDb(); const creator = await ownedCreator(ctx);
      if (input.thumbnailAssetId) await requireOwnedAsset(db, creator.id, input.thumbnailAssetId); if (input.coverAssetId) await requireOwnedAsset(db, creator.id, input.coverAssetId);
      await db.update(courses).set({ title: input.title, description: input.description, status: input.status, learningOutcomes: input.learningOutcomes, thumbnailAssetId: input.thumbnailAssetId ?? null, coverAssetId: input.coverAssetId ?? null, settings: input.settings }).where(and(eq(courses.id, input.id), eq(courses.creatorId, creator.id)));
      return { success: true };
    }),
    ...
    builder: protectedProcedure.input(z.object({ courseId: z.number().int() })).query(async ({ ctx, input }) => { const db = await requireDb(); const creator = await ownedCreator(ctx); const course = (await db.select().from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0]; if (!course) throw new TRPCError({ code: "NOT_FOUND" }); const [modules, courseLessons, assets] = await Promise.all([db.select().from(courseModules).where(eq(courseModules.courseId, course.id)).orderBy(courseModules.sortOrder), db.select().from(lessons).where(eq(lessons.courseId, course.id)).orderBy(lessons.sortOrder), db.select().from(mediaAssets).where(eq(mediaAssets.creatorId, creator.id)).orderBy(desc(mediaAssets.createdAt))]); return { course, modules, lessons: courseLessons, assets }; }),
    saveModule: protectedProcedure.input(z.object({ id: z.number().int().optional(), courseId: z.number().int(), title: z.string().min(2).max(255), description: z.string().max(8000).optional(), imageAssetId: z.number().int().nullable().optional(), status: z.enum(["draft", "published"]), isVisible: z.boolean().default(true), dripDays: z.number().int().min(0).max(3650).default(0) })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const creator = await ownedCreator(ctx); const course = (await db.select().from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0]; if (!course) throw new TRPCError({ code: "NOT_FOUND" }); if (input.imageAssetId) await requireOwnedAsset(db, creator.id, input.imageAssetId); const payload = { title: input.title, description: input.description || null, imageAssetId: input.imageAssetId ?? null, status: input.status, isVisible: input.isVisible, dripDays: input.dripDays }; if (input.id) { await db.update(courseModules).set(payload).where(and(eq(courseModules.id, input.id), eq(courseModules.courseId, course.id))); return input.id; } const count = (await db.select({ id: courseModules.id }).from(courseModules).where(eq(courseModules.courseId, course.id))).length; const result = await db.insert(courseModules).values({ ...payload, courseId: course.id, sortOrder: count }); return Number(result[0].insertId); }),
    reorderModules: protectedProcedure.input(z.object({ courseId: z.number().int(), moduleIds: z.array(z.number().int()).min(1) })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const creator = await ownedCreator(ctx); const course = (await db.select({ id: courses.id }).from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0]; if (!course) throw new TRPCError({ code: "NOT_FOUND" }); const updates = input.moduleIds.map((id, sortOrder) => db.update(courseModules).set({ sortOrder }).where(and(eq(courseModules.id, id), eq(courseModules.courseId, course.id)))); await Promise.all(updates); return { success: true }; }),
    removeModule: protectedProcedure.input(z.object({ courseId: z.number().int(), id: z.number().int() })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const creator = await ownedCreator(ctx); const course = (await db.select({ id: courses.id }).from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0]; if (!course) throw new TRPCError({ code: "NOT_FOUND" }); await db.delete(courseModules).where(and(eq(courseModules.id, input.id), eq(courseModules.courseId, course.id))); return { success: true }; }),
    addLesson: protectedProcedure.input(courseLessonInput.omit({ id: true })).mutation(async ({ ctx, input }) => {
      const db = await requireDb(); const creator = await ownedCreator(ctx);
      const course = (await db.select().from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0];
      if (!course) throw new TRPCError({ code: "NOT_FOUND" });
      await validateCourseLessonOwnership(db, creator.id, course.id, input);
      const current = await db.select({ id: lessons.id }).from(lessons).where(eq(lessons.courseId, input.courseId));
      const result = await db.insert(lessons).values({ ...input, body: input.body || null, videoUrl: input.videoUrl || null, fileUrl: input.fileUrl || null, moduleId: input.moduleId ?? null, mediaAssetId: input.mediaAssetId ?? null, thumbnailAssetId: input.thumbnailAssetId ?? null, galleryAssetIds: input.galleryAssetIds ?? null, resourceAssetIds: input.resourceAssetIds ?? null, durationSeconds: input.durationSeconds ?? null, quiz: input.quiz ?? null, prerequisiteLessonId: input.prerequisiteLessonId ?? null, sortOrder: current.length }); return Number(result[0].insertId);
    }),
    updateLesson: protectedProcedure.input(courseLessonInput.extend({ id: z.number().int() })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const creator = await ownedCreator(ctx); const course = (await db.select().from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0]; if (!course) throw new TRPCError({ code: "NOT_FOUND" }); await validateCourseLessonOwnership(db, creator.id, course.id, input); await db.update(lessons).set({ title: input.title, kind: input.kind, body: input.body || null, videoUrl: input.videoUrl || null, fileUrl: input.fileUrl || null, moduleId: input.moduleId ?? null, mediaAssetId: input.mediaAssetId ?? null, thumbnailAssetId: input.thumbnailAssetId ?? null, galleryAssetIds: input.galleryAssetIds ?? null, resourceAssetIds: input.resourceAssetIds ?? null, durationSeconds: input.durationSeconds ?? null, quiz: input.quiz ?? null, isPublished: input.isPublished, isLocked: input.isLocked, dripDays: input.dripDays, prerequisiteLessonId: input.prerequisiteLessonId ?? null, isPreview: input.isPreview }).where(and(eq(lessons.id, input.id), eq(lessons.courseId, course.id))); return { success: true }; }),
    ...
    reorderLessons: protectedProcedure.input(z.object({ courseId: z.number().int(), moduleId: z.number().int().nullable().optional(), lessonIds: z.array(z.number().int()).min(1) })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const creator = await ownedCreator(ctx); const course = (await db.select().from(courses).where(and(eq(courses.id, input.courseId), eq(courses.creatorId, creator.id))).limit(1))[0]; if (!course) throw new TRPCError({ code: "NOT_FOUND" }); if (input.moduleId) { const module = (await db.select({ id: courseModules.id }).from(courseModules).where(and(eq(courseModules.id, input.moduleId), eq(courseModules.courseId, course.id))).limit(1))[0]; if (!module) throw new TRPCError({ code: "NOT_FOUND", message: "Course module not found." }); } await Promise.all(input.lessonIds.map((id, sortOrder) => db.update(lessons).set({ sortOrder, moduleId: input.moduleId ?? null }).where(and(eq(lessons.id, id), eq(lessons.courseId, course.id))))); return { success: true }; }),
  ...
  account: router({
    ...
    course: protectedProcedure.input(z.object({ courseId: z.number().int() })).query(async ({ ctx, input }) => { const db = await requireDb(); const row = (await db.select().from(enrollments).innerJoin(customers, eq(enrollments.customerId, customers.id)).innerJoin(courses, eq(enrollments.courseId, courses.id)).where(and(eq(customers.userId, ctx.user.id), eq(enrollments.courseId, input.courseId))).limit(1))[0]; if (!row) throw new TRPCError({ code: "FORBIDDEN", message: "You are not enrolled in this course." }); const [modules, courseLessons, attempts] = await Promise.all([db.select().from(courseModules).where(eq(courseModules.courseId, row.courses.id)).orderBy(courseModules.sortOrder), db.select().from(lessons).where(eq(lessons.courseId, row.courses.id)).orderBy(lessons.sortOrder), db.select().from(lessonQuizAttempts).where(eq(lessonQuizAttempts.enrollmentId, row.enrollments.id)).orderBy(desc(lessonQuizAttempts.createdAt))]); const completed = row.enrollments.completedLessonIds ?? []; const moduleById = new Map(modules.map((module) => [module.id, module])); const referencedAssetIds = new Set(courseLessons.flatMap((lesson) => [lesson.mediaAssetId, lesson.thumbnailAssetId, ...(lesson.galleryAssetIds ?? []), ...(lesson.resourceAssetIds ?? [])]).filter((id): id is number => id !== null)); const creatorAssets = referencedAssetIds.size ? await db.select().from(mediaAssets).where(eq(mediaAssets.creatorId, row.courses.creatorId)) : []; return { course: row.courses, enrollment: row.enrollments, modules, lessons: courseLessons.map((lesson) => ({ ...lesson, availability: studentLessonAvailability(lesson, lesson.moduleId ? moduleById.get(lesson.moduleId) : undefined, row.enrollments, completed) })), assets: creatorAssets.filter((asset) => referencedAssetIds.has(asset.id)), quizAttempts: attempts }; }),
    completeLesson: protectedProcedure.input(z.object({ courseId: z.number().int(), lessonId: z.number().int() })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const row = (await db.select().from(enrollments).innerJoin(customers, eq(enrollments.customerId, customers.id)).where(and(eq(customers.userId, ctx.user.id), eq(enrollments.courseId, input.courseId))).limit(1))[0]; if (!row) throw new TRPCError({ code: "FORBIDDEN", message: "You are not enrolled in this course." }); const [lesson, modules, courseLessons] = await Promise.all([(await db.select().from(lessons).where(and(eq(lessons.id, input.lessonId), eq(lessons.courseId, input.courseId))).limit(1))[0], db.select().from(courseModules).where(eq(courseModules.courseId, input.courseId)), db.select({ id: lessons.id }).from(lessons).where(and(eq(lessons.courseId, input.courseId), eq(lessons.isPublished, true)))]); if (!lesson) throw new TRPCError({ code: "NOT_FOUND", message: "Lesson not found." }); const availability = studentLessonAvailability(lesson, lesson.moduleId ? modules.find((module) => module.id === lesson.moduleId) : undefined, row.enrollments, row.enrollments.completedLessonIds ?? []); if (!availability.isAvailable) throw new TRPCError({ code: "FORBIDDEN", message: availability.reason || "This lesson is locked." }); if (lesson.kind === "quiz") throw new TRPCError({ code: "BAD_REQUEST", message: "Submit the quiz to complete this lesson." }); const completedLessonIds = mergeCompletedLesson(row.enrollments.completedLessonIds ?? [], input.lessonId); await db.update(enrollments).set({ completedLessonIds, completedAt: completedLessonIds.length === courseLessons.length ? new Date() : null }).where(eq(enrollments.id, row.enrollments.id)); return { completedLessonIds }; }),
    submitQuiz: protectedProcedure.input(z.object({ courseId: z.number().int(), lessonId: z.number().int(), selectedAnswerIndex: z.number().int().min(0).max(7) })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const row = (await db.select().from(enrollments).innerJoin(customers, eq(enrollments.customerId, customers.id)).where(and(eq(customers.userId, ctx.user.id), eq(enrollments.courseId, input.courseId))).limit(1))[0]; if (!row) throw new TRPCError({ code: "FORBIDDEN", message: "You are not enrolled in this course." }); const [lesson, modules, courseLessons] = await Promise.all([(await db.select().from(lessons).where(and(eq(lessons.id, input.lessonId), eq(lessons.courseId, input.courseId))).limit(1))[0], db.select().from(courseModules).where(eq(courseModules.courseId, input.courseId)), db.select({ id: lessons.id }).from(lessons).where(and(eq(lessons.courseId, input.courseId), eq(lessons.isPublished, true)))]); if (!lesson || lesson.kind !== "quiz" || !lesson.quiz) throw new TRPCError({ code: "NOT_FOUND", message: "Quiz lesson not found." }); const availability = studentLessonAvailability(lesson, lesson.moduleId ? modules.find((module) => module.id === lesson.moduleId) : undefined, row.enrollments, row.enrollments.completedLessonIds ?? []); if (!availability.isAvailable) throw new TRPCError({ code: "FORBIDDEN", message: availability.reason || "This lesson is locked." }); if (input.selectedAnswerIndex >= lesson.quiz.choices.length) throw new TRPCError({ code: "BAD_REQUEST", message: "Choose a valid quiz answer." }); const isPassed = input.selectedAnswerIndex === lesson.quiz.correctAnswerIndex; const score = isPassed ? 100 : 0; await db.insert(lessonQuizAttempts).values({ enrollmentId: row.enrollments.id, lessonId: lesson.id, selectedAnswerIndex: input.selectedAnswerIndex, score, isPassed }); const completedLessonIds = isPassed ? mergeCompletedLesson(row.enrollments.completedLessonIds ?? [], lesson.id) : row.enrollments.completedLessonIds ?? []; if (isPassed) await db.update(enrollments).set({ completedLessonIds, completedAt: completedLessonIds.length === courseLessons.length ? new Date() : null }).where(eq(enrollments.id, row.enrollments.id)); return { score, isPassed, explanation: lesson.quiz.explanation ?? null, completedLessonIds }; }),
